...
VALIDATE_MINIMUM_ERROR = (
    "The response value {data} is lower than the specified minimum of {minimum}"
)
VALIDATE_MAXIMUM_ERROR = "The response value {data} exceeds the maximum allowed value of {maximum}"
VALIDATE_MIN_LENGTH_ERROR = (
    'The length of "{data}" is shorter than the specified minimum length of {min_length}'
)
...
VALIDATE_MAX_ARRAY_LENGTH_ERROR = (
    "The length of the array {data} exceeds the specified maximum length of {max_length}"
)
VALIDATE_MINIMUM_NUMBER_OF_PROPERTIES_ERROR = (
    "The number of properties in {data} is fewer than the specified "
    "minimum number of properties of {min_length}"
)
VALIDATE_MAXIMUM_NUMBER_OF_PROPERTIES_ERROR = (
    "The number of properties in {data} exceeds the specified "
    "maximum number of properties of {max_length}"
)
...
VALIDATE_WRITE_ONLY_RESPONSE_KEY_ERROR = (
    ...
    'documented as being "writeOnly": "{write_only_key}"'
)
VALIDATE_ONE_OF_ERROR = (
    "Expected data to match one and only one of the oneOf schema types; found {matches} matches"
)
